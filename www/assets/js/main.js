...
(function () {
	...
	var bondClicked = function (dataPoint) {
		...
		// Send to PB what's the selected atom
		if(window.external){
			window.external.JsTrigger("boundClicked->" + bondSelected[0][0].parentNode.__data__.id);
		}
	};
	...
	window.loadMoleculeText = function(data){
		newMoleculeSimulation(JSON.parse(data));
	}
	...
	var orgoShmorgo = function(graph) {
	  ...
	  function addNewAtom (atomType, atomSize) {
			...
			// Send to PB what's the selected atom
			if(window.external){
				window.external.JsTrigger("atomAdded->" + JSON.stringify(newAtom));
			}
		  ...
		  links.push(link); // Need to make sure is unique
		  
			// Send to PB what's the selected atom
			if(window.external){
				window.external.JsTrigger("boundAdded->" + JSON.stringify({
